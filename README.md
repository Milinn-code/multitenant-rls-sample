...
| `app_user` | 可 | 業務テーブルの読み書き（`id` 列は書けない）。テナント（`tenants`）は読み取りだけ。監査ログには権限がない。RLS を外せない。一時テーブルも作れない |
...
| 8 | `app_user` は RLS を無効にできず、ポリシーも変えられず、テーブルの所有者でもない。`tenant_id` 列を持つテーブルは（新しく追加したものも含めて）すべて RLS が有効で、行を許可するポリシーはテナントの条件の1つだけ。`id` の指定や一時テーブルの作成もできない | `roles.test.ts` |
...
| 新しいテーブルで RLS を付け忘れる、すべてを許可するポリシー（`USING (true)` など）を置く | `tenant_id` 列を持つテーブルをカタログから列挙し、RLS の有効化に加えて、行を許可する（PERMISSIVE な）ポリシーがテナントの条件の `tenant_isolation` 1つだけであることを、条件式まで検査するテスト | 8 |
...
| 監査ログから、操作した人を特定する | 監査ログの「誰が」（`actor`）に残るのは、接続したロール名（共用の `ops_user`）で、実際に操作した人ではありません。人まで特定するには、運営者ごとにロールを分けるか、アプリ側で認証した利用者を関数の引数として渡して記録する必要があります |
